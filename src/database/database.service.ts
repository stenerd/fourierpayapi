...
export class DatabaseService implements OnModuleInit {
    ...
    async seedDb() {
        ...
        if (!institution) {
            ...
            if (!findFaculty) {
                const newFacultyMetadata = await this.metadata.createMetadata(Roles.FACULTY, { name: faculty[0], slug: faculty[0] });
                const findFaculty = await this.facultyMetadata.findOne({ name: faculty[0] })
                ...
                if (!departmentMeta) {
                    const newDepartment = await this.metadata.createMetadata(Roles.DEPARTMENT, { name: department[0], slug: department[0] })
                    const departmentMeta = await this.departmentMetadata.findOne({ name: department[0] })
